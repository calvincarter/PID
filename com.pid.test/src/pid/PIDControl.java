...
public class PIDControl {
	...
	{ 
	  float dt=(float)(System.currentTimeMillis()-heading_PID_timer)/1000;//calculating dt, you must divide it by 1000, because this system only undestands seconds.. and is normally given in millis


	  //Integratior part
	  Start.heading_I+= (float)PID_error*(float)dt; //1000 microseconds / 1000 = 1 millisecond
	  Start.heading_I=constrain(Start.heading_I,Start.heading_min,Start.heading_max); //Limit the PID integrator... 

	  //Derivation part
	  heading_D=((float)PID_error-(float) Start.heading_previous_error)/(float)dt;

	  heading_output=0;//Clearing the variable.	

	  heading_output=(int)(Start.kp[0]*PID_error);//Proportional part, is just the KP constant * error.. and addidng to the output 
